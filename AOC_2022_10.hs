...
count2 (x : xs) reg cycle crt = let d = take 4 x
                                    s = if (d == "addx") then (read (drop 5 x)) :: Int else 0
                                        in case d of "addx" -> let t1 = if (((cycle) `mod` 40 >= reg - 1) && ((cycle) `mod` 40 <= reg                           + 1)) then "#" else "."
                                                                   t2 = if (((cycle + 1)`mod` 40 >= reg - 1) && ((cycle + 1)`mod` 40 <= reg + 1)) then "#" else "."
                                                                        in count2 xs (reg + s) (cycle + 2) (crt ++ t1 ++ t2)
                                                     "noop" -> let t1 = if (((cycle) `mod` 40 >= reg - 1) && ((cycle) `mod` 40 <= reg                   + 1)) then "#" else "."

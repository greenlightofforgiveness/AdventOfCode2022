...
analyze1 :: String -> Int
analyze1 s = let lst = (map (\x -> (read x :: Int) * 811589153) (lines s))
                 n = length lst
...
count1 lst n i = if (i == 10 * n) then let x = find1 lst
                                           x1 = (x + 1000) `mod` n
                                           x2 = (x + 2000) `mod` n
                                           x3 = (x + 3000) `mod` n
                                                in find2 x1 x2 x3 lst 0
                else let  (a', b') = lst !! (i `mod` n)

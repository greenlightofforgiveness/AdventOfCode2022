...
count1 m items insp m_lcm 80000 = let x = reverse $ sort $ Map.elems insp in (x !! 0) * (x !! 1)
count1 m items insp m_lcm i = let n = i `mod` 8
                                  f1 = fst (m !! n)
                                  f2 = snd (m !! n)
                                  f3 = (\x -> x `mod` m_lcm)
                                  items' = items Map.! n
                                ...
                                in count1 m (insert' mov items'' (Map.insert n [] items)) (Map.insertWith (+) n (length items') insp) m_lcm (i + 1)
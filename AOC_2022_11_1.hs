...
analyze1 :: String
analyze1 = show (count1 monkeys_func monkeys_items monkeys_insp 0)
...
count1 :: [(Int -> Int, Int -> Int)] -> Map.Map Int [Int] -> Map.Map Int Int -> Int -> Int
count1 m items insp 160 = let x = reverse $ sort $ Map.elems insp in (x !! 0) * (x !! 1)
count1 m items insp i = let n = i `mod` 8
                            f1 = fst (m !! n)
                            f2 = snd (m !! n)
                            f3 = \x -> x `div` 3
                            items' = items Map.! n
                            items'' = map (f3.f1) items'
                            mov = map (f2.f3.f1) items'
                                in count1 m (insert' mov items'' (Map.insert n [] items)) (Map.insertWith (+) n (length items') insp) (i + 1)